...
metrics = ['completeness', 'homogeneity', 'Mutual Info', 'Fowlkes Mallows', 'davies bouldin','calisnki']
#scores = [[]]*len(metrics)
scores = [[], [], [], [], [], []]
rounds = 20
print(scores)
for k in range(2, 24):
    ...
    for j in range(rounds):    
        ...
        dbs += davies_bouldin_score(kdd.data.values, labels=y_pred)
        chs += calinski_harabasz_score(kdd.data.values, y_pred)
...
for i in range(len(metrics)-1):
    plt.plot(np.arange(0,len(scores[0])), scores[i], label = metrics[i])
